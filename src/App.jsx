...
import {
  ...
  Headphones, Eye, EyeOff, PenLine, Dumbbell, UserPlus, Lock, X, Zap,
  Waves, Share2, MessageCircle, RefreshCw
} from "lucide-react";
...
const METHODOLOGIES = {
  pomodoro: {
    title: "Pomodoro",
    icon: Timer,
    guide: "Trabaja 25 minutos sin interrupciones y descansa 5. Cada 4 bloques, toma un descanso largo de 15-20 minutos.",
  },
  flowtime: {
    title: "Flowtime",
    icon: Waves,
    guide: "Estudia sin cronómetro fijo hasta notar que tu concentración baja, anota cuánto duró el bloque y descansa un tiempo proporcional antes de retomar.",
  },
  feynman: {
    title: "Técnica Feynman",
    icon: MessageCircle,
    guide: "Explica el tema en voz alta con tus propias palabras, como si se lo enseñaras a alguien que no sabe nada. Donde te trabes, vuelve al material y simplifica.",
  },
  mapas_mentales: {
    title: "Mapas Mentales",
    icon: Share2,
    guide: "Escribe el concepto principal al centro de la hoja y desprende ramas con ideas relacionadas, usando colores y dibujos para conectar la información.",
  },
  cornell: {
    title: "Método Cornell",
    icon: PenLine,
    guide: "Divide la hoja en apuntes, palabras clave y resumen. Toma notas en clase, agrega palabras clave después y escribe un resumen al final de la página.",
  },
  sq3r: {
    title: "SQ3R",
    icon: BookOpen,
    guide: "Explora el texto, hazte preguntas sobre el contenido, léelo a fondo, recita lo que recuerdas sin mirar y repasa tus apuntes al final.",
  },
  flashcards: {
    title: "Fichas de repaso",
    icon: RefreshCw,
    guide: "Crea tarjetas con una pregunta por un lado y la respuesta por el otro. Repásalas con repetición espaciada, revisando más seguido las que más te cuesten.",
  },
};

const METHOD_RECOMMENDATIONS = {
  visual: ["mapas_mentales", "cornell", "flashcards"],
  auditivo: ["feynman", "pomodoro", "flowtime"],
  lectura: ["cornell", "sq3r", "flashcards"],
  kinestesico: ["pomodoro", "flowtime", "feynman"],
};

const ACHIEVEMENT_DEFS = [
  { id: "first_session", title: "Primer Enfoque", desc: "Completa tu primera sesión Pomodoro", icon: Timer, color: "#1CB0F6",
    check: (s) => s.sessionsCompleted >= 1 },
  { id: "explorer", title: "Autoconocimiento", desc: "Descubre tu estilo de aprendizaje", icon: Brain, color: "#8549BA",
    check: (s) => s.quizCompleted },
  { id: "streak3", title: "Racha de 3", desc: "Estudia 3 días seguidos", icon: Flame, color: "#FF9600",
    check: (s) => s.streak >= 3 },
  { id: "marathon", title: "Maratonista", desc: "Acumula 100 minutos de estudio", icon: Sparkles, color: "#58CC02",
    check: (s) => s.totalMinutes >= 100 },
  { id: "social", title: "En Comunidad", desc: "Agrega a un amigo", icon: Users, color: "#FF4B4B",
    check: (s) => s.friendsAdded >= 1 },
  { id: "collector", title: "Coleccionista", desc: "Acumula 200 monedas", icon: Coins, color: "#FFC800",
    check: (s) => s.coins >= 200 },
];
...
export default function StudyQuestApp() {
  ...
  // Aviso visual inmediato: soporta texto + insignias con ícono propio (XP, monedas, logros)
  const showToast = ({ text, icon = Sparkles, color = "#FFC800", badges = [] }) => {
    setToast({ text, icon, color, badges });
    window.clearTimeout(showToast._t);
    showToast._t = window.setTimeout(() => setToast(null), 2800);
  };

  // Detecta logros recién desbloqueados para avisar con el ícono propio de cada Misión Académica
  const prevUnlockedRef = useRef(null);
  useEffect(() => {
    const currentIds = ACHIEVEMENT_DEFS.filter((a) => a.check(stats)).map((a) => a.id);
    if (prevUnlockedRef.current !== null) {
      const newlyUnlocked = currentIds.filter((id) => !prevUnlockedRef.current.includes(id));
      if (newlyUnlocked.length > 0) {
        const achievement = ACHIEVEMENT_DEFS.find((a) => a.id === newlyUnlocked[0]);
        if (achievement) {
          showToast({
            text: `¡Misión cumplida: ${achievement.title}!`,
  ...
  return (
    <div className="min-h-screen bg-[#F7F9F7] flex flex-col items-center font-sans">
      <div className="w-full max-w-sm bg-[#F7F9F7] min-h-screen flex flex-col relative shadow-xl">
        ...
        {/* Toast */}
        {toast && (
          <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 bg-gray-800 text-white text-xs font-semibold pl-3 pr-2 py-2 rounded-xl shadow-lg flex items-center gap-2 max-w-[90%] animate-pulse">
            <toast.icon size={16} color={toast.color} className="shrink-0" />
            <span className="leading-tight">{toast.text}</span>
            {toast.badges.map((b, i) => {
              const BadgeIcon = b.icon;
              return (
                <span
                  key={i}
                  className="flex items-center gap-0.5 text-[11px] font-extrabold px-1.5 py-0.5 rounded-full shrink-0"
                  style={{ backgroundColor: b.color + "33", color: b.color }}
                >
                  <BadgeIcon size={11} /> {b.label}
                </span>
              );
            })}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-24 pt-2">
          {tab === "home" && (
            <HomeTab stats={stats} nextLevelXp={nextLevelXp} learningStyle={learningStyle}
              unlocked={unlockedAchievements} setTab={setTab} />
          )}
          {tab === "pomodoro" && (
          ...
          {tab === "friends" && (
            <FriendsTab stats={stats} setStats={setStats} friends={friends} setFriends={setFriends}
              showToast={showToast} socialHidden={socialHidden} setSocialHidden={setSocialHidden} />
          )}
        </div>
...
// ---------- Home ----------
function HomeTab({ stats, nextLevelXp, learningStyle, unlocked, setTab }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <p className="font-bold text-gray-800 text-sm mb-3">Metodologías recomendadas para ti</p>
        {learningStyle ? (
          <div className="flex flex-col gap-3">
            {METHOD_RECOMMENDATIONS[learningStyle].map((id) => {
              const m = METHODOLOGIES[id];
              const MIcon = m.icon;
              return (
                <div key={id} className="flex gap-3 items-start">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0"
                    style={{ backgroundColor: STYLE_INFO[learningStyle].color + "1F" }}>
                    <MIcon size={16} color={STYLE_INFO[learningStyle].color} />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-gray-700">{m.title}</p>
                    <p className="text-[11px] text-gray-500 leading-relaxed mt-0.5">{m.guide}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="flex flex-col items-center text-center gap-2 py-2">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Descubre tu estilo de aprendizaje para desbloquear una lista de metodologías de estudio hechas para ti.
            </p>
            <button onClick={() => setTab("quiz")}
              className="mt-1 bg-[#8549BA] text-white text-xs font-bold px-4 py-2 rounded-full">
              Hacer test para desbloquear
            </button>
          </div>
        )}
      </div>
  ...
  const handleComplete = () => {
    ...
    if (mode === "work") {
      ...
      showToast({
        ...
        color: "#FFC800",
        badges: [
          { icon: Zap, label: `+${pointsEarned} XP`, color: "#1CB0F6" },
          { icon: Coins, label: `+${coinsEarned}`, color: "#FFC800" },
        ],
      });
      setMode("break");
      setSecondsLeft(BREAK_MIN * 60);
    } else {
      showToast({ text: "Descanso terminado. ¡A por otra sesión!", icon: Timer, color: "#1CB0F6" });
      setMode("work");
    ...
    if (step + 1 < QUIZ_QUESTIONS.length) {
      ...
      if (!stats.quizCompleted) {
        ...
        showToast({
          ...
          icon: Brain,
          color: "#8549BA",
          badges: [{ icon: Zap, label: "+30 XP", color: "#1CB0F6" }],
        });
      }
    }
  };
          ...
          return (
            <div key={a.id}
              ...
              <div className="w-10 h-10 rounded-full flex items-center justify-center mb-2"
                style={{ backgroundColor: unlocked ? a.color + "22" : "#E5E7EB" }}>
                {unlocked ? <Icon size={18} color={a.color} /> : <Lock size={16} color="#9CA3AF" />}
              </div>
...
// ---------- Friends ----------
function FriendsTab({ stats, setStats, friends, setFriends, showToast, socialHidden, setSocialHidden }) {
  ...
  const addFriend = () => {
    ...
    showToast({ text: `${name.trim()} se unió a tu grupo de estudio 🤝`, icon: UserPlus, color: "#58CC02" });
    setName("");
    setShowAdd(false);
  };
  ...
  return (
    <div className="flex flex-col gap-3 pt-2">
      ...
      <button
        onClick={() => setSocialHidden((h) => !h)}
        className="flex items-center justify-between bg-white rounded-2xl p-3 shadow-sm border border-gray-100"
      >
        <span className="flex items-center gap-2 text-xs font-semibold text-gray-600">
          {socialHidden ? <EyeOff size={16} color="#9CA3AF" /> : <Eye size={16} color="#58CC02" />}
          {socialHidden ? "Ranking social oculto" : "Ranking social visible"}
        </span>
        <span className="text-[11px] font-bold text-[#1CB0F6]">
          {socialHidden ? "Mostrar" : "Ocultar"}
        </span>
      </button>

      {socialHidden ? (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center">
          <EyeOff size={22} color="#9CA3AF" className="mx-auto mb-2" />
          <p className="text-xs text-gray-400 leading-relaxed">
            Ocultaste el ranking para estudiar sin comparar con otros. Actívalo cuando quieras verlo de nuevo.
          </p>
        </div>
      ) : (
